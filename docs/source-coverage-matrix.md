...
| Binance USD-M | open-interest history | `GET /futures/data/openInterestHist` | latest one month; documented request limits | none | `source-probe` target `binance_usdm_open_interest_history` | yes | probe candidate; typed persistence through CCXT first | source health only |
| Binance USD-M | premium-index klines | `GET /fapi/v1/premiumIndexKlines` | weight varies by limit | none | `source-probe` target `binance_usdm_premium_index_klines` | yes | probe candidate | source health only |
| Binance USD-M | basis | `GET /futures/data/basis` | recent history window | none | `source-probe` target `binance_usdm_basis` | yes | probe candidate | source health only |
| Binance USD-M | global account long/short ratio | `GET /futures/data/globalLongShortAccountRatio` | recent history window | none | `source-probe` target `binance_usdm_global_long_short_account_ratio` | yes | probe candidate for Phase 7 source health | source health only |
