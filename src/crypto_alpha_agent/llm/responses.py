...
def _schema_hint_for_task(task: Any) -> str:
    ...
    if task_type == "ExperimentPlannerTask":
        return (
            ...
            "selected validator's required_record_types. why_it_might_improve_edge "
            "and expected_edge_mechanism must each be one single string, not a "
            "list or object. parameter_changes must be a JSON object. Use only "
    ...
    if task_type == "LLMJudgementTask":
        return (
            ...
            "and live_order_routing=false. Use decision exactly one of: "
            "useful_for_research, not_ready, ready_for_offline_research, "
            "research_only, research_ready, keep_collecting, add_data, "
            "redesign_validator, stop, owner_decision_review, blocked. "
            "Do not use synonyms such as useful. For DataReadinessJudgement include "
